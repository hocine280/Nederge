...
include "../../vendor/autoload.php";
    ...
    $nbServers = count($_SESSION['listServer']->getListTARE());
    $i = 0;
    foreach($_SESSION['listServer']->getListTARE() as $port=>$name){
        if(isset($_POST['loginOrder']) && !empty($_POST['loginOrder'])){
            
            $urlStatusOrder = "http://localhost:".$port."/order-status";
            ...
            if(($jsonReceivedStatusOrder = @file_get_contents($urlStatusOrder, false, $contextStatusOrder))!== false){
                ...
                if($json['status'] == true && $json['idOrder'] == $_POST['idOrderForm']){
                    ...
                    $url = "http://localhost:".$port."/remove-order";
